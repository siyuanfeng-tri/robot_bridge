...
public:
  ...
  Eigen::Matrix3d ComputeEscapeFrame(const Eigen::Vector3d& closest, const Eigen::Vector3d& point) const;

  double GetClosestPointsOnAxis(const Capsule& other,
      Eigen::Vector3d* my_point,
      Eigen::Vector3d* other_point) const;

  const RigidBody<double>& get_body() const {
    return X_BC_.get_rigid_body();
  }
